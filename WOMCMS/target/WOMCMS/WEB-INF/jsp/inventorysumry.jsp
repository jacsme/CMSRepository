...
<script>
    ...
    function SummaryController($scope, $http, $modal, $log){
    	$scope.open = function (inventorylist) {
	...
	angular.module('inventoryapp').controller('ModalInstanceCtrl', function ($scope, $http, $modalInstance, inventorylist) {
		$scope.loading2 = true;
		$scope.inventorylisting = inventorylist;
    	$scope.exportData = function () {
            var blob = new Blob([document.getElementById('exportable').innerHTML], {
                type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;charset=utf-8"
            });
            saveAs(blob, "WOM_Inventory_Summay.xls");
...
<div class="container">
	...
	<div data-ng-controller="SummaryController" class="my-controller">
		<script type="text/ng-template" id="inventoryModal.html">
   			<div id="exportable" style="width: 30px; height: 40px">
			<table id="inventory" style="display:none">
				<thead>
					<tr>
						<th>No</th>
						<th>Date</th>
				...
				<tbody>
					<tr data-ng-repeat="inventory in inventorylisting">
					   	<td>{{ $index + 1 }}</td>
					    ...
					    <td>{{inventory.rrprice}}</td>
					    <td>{{inventory.unitsAll}}</td>
					    <td>{{inventory.photo}}</td>
					 </tr>
				</tbody>
			</table>
			</div>
			...
			<div class="modal-footer" style="text-align: center">
   				<button class="btn btn-warning" type="button" data-ng-click="exportData()">Export</button>
			...
			<table id="product">
				...
				<tbody>
					<tr data-dir-paginate="inventory in inventorylist | filter:q | itemsPerPage: pageSize" data-current-page="currentPage">
					    ...
					    <td><span data-editable-text="inventory.unitsAll" data-e-name="units" data-e-form="rowform" data-e-style="width: 40px" data-e-disabled="disabled">
					    {{ inventory.unitsAll }} </span></td>
		...
		<table>
			<tr>
		        <td colspan="2" style="text-align:center">
		        	<button class="button-blue" data-ng-click="open(inventorylist);">Export to Excel</button>
		        </td>
	        </tr>
	 	</table>
    </div>	
